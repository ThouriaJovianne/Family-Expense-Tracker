...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.revenue.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.revenues.update", [$revenue->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name_of_source">{{ trans('cruds.revenue.fields.name_of_source') }}</label>
                <input class="form-control {{ $errors->has('name_of_source') ? 'is-invalid' : '' }}" type="text" name="name_of_source" id="name_of_source" value="{{ old('name_of_source', $revenue->name_of_source) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="number" name="amount" id="amount" value="{{ old('amount', $revenue->amount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_of_receipt') ? 'is-invalid' : '' }}" type="text" name="date_of_receipt" id="date_of_receipt" value="{{ old('date_of_receipt', $revenue->date_of_receipt) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $revenue->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
